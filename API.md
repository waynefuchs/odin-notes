...
I find this is one of the more interesting (and frustrating) topics in all of the Odin work that I've worked on.
...
They are best used to verify that the data contained in an object is genuine and not tampered with. But an SSL site already has this feature built-in. XSS becomes an issue if stored improperly. Overhead increases, especially when used in conjunction with cookies. The JWT can not be revoked.
...
| Action  | Verb   | Method Verb | API | HTML Route | PATH                   | Task                               |
| ------- | ------ | ----------- | --- | ---------- | ---------------------- | ---------------------------------- |
