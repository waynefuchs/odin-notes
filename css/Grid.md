...
> 🚧 TODO: Is this useful to anyone? Remove?
...
> 🚧 TODO: This would be better in a table.

## Grid Container

The parent element, or the element providing `display: grid;`. Containers can be nested, but the children (Grid Items) will always look to the closest parent in the hierarchy.
...
The dividing lines between grid items, either _column grid lines_ (horizontal) or _row grid lines_ (vertical).
...
Grid will use the templates it has been provided to overflow any 'cells' that have not been defined _implicitly_. Size values that have been established using grid-template(x) will not be carried over into implicit grid tracks. To define those tracks, you must use grid-auto(x).
